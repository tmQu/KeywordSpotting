...
static void CaptureSamples(void* arg) {
  ...
  while (1) {
    ...
    // Serial.printf("Time taken to read %d bytes: %d ms\n", bytes_read, now - timer);
    // timer = now;
    if (bytes_read <= 0) {
      ...
      // ESP_LOGD(TAG, "%d timestamp bytes written: %d bytes read %d, ring buff fill %d\n", g_latest_audio_timestamp, bytes_written, bytes_read, rb_filled(g_audio_capture_buffer));
      if (bytes_written <= 0) {
        ESP_LOGE(TAG, "Could Not Write in Ring Buffer: %d ", bytes_written);
        ESP_LOGE(TAG, "Ring Buffer Filled: %d ", rb_filled(g_audio_capture_buffer));
...
TfLiteStatus InitAudioRecording(tflite::ErrorReporter* error_reporter) {
   ...
   * in the ring buffer */
  ESP_LOGI(TAG, "Creating CaptureSamples Task");
  xTaskCreate(CaptureSamples, "CaptureSamples", 1024 * 20, NULL, 1, NULL);
  // xTaskCreatePinnedToCore(CaptureSamples, "CaptureSamples", 1024*20, NULL, 5, NULL, 0);
...
TfLiteStatus GetAudioSamples(tflite::ErrorReporter* error_reporter,
  ...
  // ESP_LOGD(TAG, "BF READ RB FILLED RIGHT NOW IS %d", rb_filled(g_audio_capture_buffer));
      // ESP_LOGD(TAG, "Xin reading, millis %d, RB FILL %d", millis(), rb_filled(g_audio_capture_buffer));
  ...
  // ESP_LOGD(TAG, "RB FILLED RIGHT NOW IS %d, read byte %d, buff len %d", rb_filled(g_audio_capture_buffer), bytes_read, new_samples_to_get * sizeof(int16_t));
  if (bytes_read < 0) {
