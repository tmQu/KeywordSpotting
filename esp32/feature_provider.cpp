...
int FeatureProvider::PopulateFeatureData(
  ...
  if (is_first_run_) {
    ...
    // if (time_in_ms - last_time_in_ms < kFeatureSliceDurationMs) {
    //   // If we're not on a new step, we don't need to fill any new slices.
    //   slices_needed = 0;

    // }
  }
  if (slices_needed == 0) {
    // If we don't need any new slices, we can just return.
  ...
  *how_many_new_slices = slices_needed;
  ...
  // +-----------+             +-----------+
  if (slices_to_keep > 0) {
    ESP_LOGI("NEW_SLICES", "Keeping %d slices", slices_to_keep);
    for (int dest_slice = 0; dest_slice < slices_to_keep; ++dest_slice) {
      float* dest_slice_data =
          feature_data_ + (dest_slice * kFeatureSliceSize * sizeof(float));
      const int src_slice = dest_slice + slices_to_drop;
      const float* src_slice_data =
          feature_data_ + (src_slice * kFeatureSliceSize * sizeof(float));
  ...
  if (slices_needed > 0) {
    for (int new_slice = slices_to_keep; new_slice < kFeatureSliceCount;
      ...
      if (audio_samples_size == 0)
      {
        *how_many_new_slices = 0;
        return 0;
      }
      bytes_read += audio_samples_size;
      if(DEBUG)
        Serial.printf("GetAudioSamples: %d ms\n", millis() - start);
      start = millis();
      ...
      float* new_slice_data = feature_data_ + (new_slice * kFeatureSliceSize * sizeof(float));
      size_t num_samples_read;
      TfLiteStatus generate_status = GenerateMicroFeatures(
          error_reporter, audio_samples, audio_samples_size, kFeatureSliceSize,
          new_slice_data, &num_samples_read);
      if(DEBUG)
        Serial.printf("GenerateMicroFeatures: %d ms\n", millis() - start);
      start = millis();
