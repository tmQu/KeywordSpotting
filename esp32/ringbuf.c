...
int rb_read(ringbuf_t* rb, uint8_t* buf, int buf_len, uint32_t ticks_to_wait) {
    ESP_LOGD(RB_TAG, "rb filled %d bytes, rb available %d bytes", 0, 0);
   ...
   * memcpy loop.
   */
  ESP_LOGD(RB_TAG, "BEGIN RING BUF rb filled %d bytes, rb available %d bytes", 11, 12);
  if (rb == NULL || rb->abort_read == 1) {
    return ESP_FAIL;
  }
  ESP_LOGD(RB_TAG, "before taking lock, buff len %d", buf_len);

  xSemaphoreTake(rb->lock, portMAX_DELAY);

  ESP_LOGD(RB_TAG, "after taking lock, buff len %d", buf_len);
  while (buf_len) {
    ...
    ESP_LOGD(RB_TAG, "rb filled %d bytes, rb available %d bytes", 100, 200);
