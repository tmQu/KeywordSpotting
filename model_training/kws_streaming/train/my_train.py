...
def train(flags, X_train, y_train, X_test, y_test, model = None):
  """Model training."""
 ...
 # Create checkpoint manager
  ...
  # checkpoint_manager = tf.train.CheckpointManager(checkpoint, directory=flags.train_dir, max_to_keep=3)

  # if checkpoint_manager.latest_checkpoint:
  #     checkpoint.restore(checkpoint_manager.latest_checkpoint)
  #     logging.info(f'Restored from {checkpoint_manager.latest_checkpoint}')
  #     start_step = int(checkpoint_manager.latest_checkpoint.split('-')[-1]) + 1
  # else:
...
#     f.write('\n'.join(audio_processor.words_list))
  ...
  for training_step in range(start_step, training_steps_max + 1):
    offset = (training_step -
              1) * 100 if flags.pick_deterministically else 0
    # Pull the audio samples we'll use for training.
    train_fingerprints, train_ground_truth = X_train, y_train
    ...
    if (training_step % flags.eval_step_interval) == 0 or is_last_step:
      ...
      set_size = int(set_size / 100) * 100
      total_accuracy = 0.0
      count = 0.0
      for i in range(0, set_size, 100):
        validation_fingerprints, validation_ground_truth = X_test, y_test
  ...
  for i in range(0, set_size, 100):
    test_fingerprints, test_ground_truth = X_test, y_test
